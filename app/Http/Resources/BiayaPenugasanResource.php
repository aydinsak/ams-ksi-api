...
class BiayaPenugasanResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        $rkia  = $this->rel($summary, 'rkia');
        $objek = $this->rel($summary, 'object');

        return [
            'id'             => $this->id,
            'no_program'     => $this->no_program ?? null,
            ...
            'program_audit'  => $this->no_program ?? null,
            'auditor'        => $this->buildAuditors($assignment),
            'rev'            => (int) $this->version,

            // include opsional (contoh)
            'summary' => $this->when(
                $request->boolean('include_summary') && $summary,
                [
                    'id'         => $summary->id,
                    'type_audit' => optional($this->rel($summary, 'type'))->name,
